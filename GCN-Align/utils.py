...
def load_data2(data_dir):
    ...
    # Es, As, Ts, ill = names
    e = len(set(loadfile(os.path.join(data_dir, "ent_ids_1"), 1)) | set(loadfile(os.path.join(data_dir, "ent_ids_2"), 1)))
    ...
    # ent2id = get_ent2id([os.path.join(data_dir, "ent_ids_1"), os.path.join(data_dir, "ent_ids_2")])
    # ILL = loadfile(os.path.join(data_dir, "ref_ent_ids"), 2)
    # attr = loadattr([As[0], As[1]], e, ent2id)
    # ae_input = get_ae_input(attr)
    adj = get_weighted_adj(e, KG)  # nx.adjacency_matrix(nx.from_dict_of_lists(get_dic_list(e, KG)))
    return adj, e, train_links, test_links
