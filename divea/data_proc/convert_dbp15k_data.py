...
def transform_dbp15k_to_uniform(data_dir, kgids, out_dir):
    kg1_id, kg2_id = kgids

    with open(os.path.join(data_dir, "s_triples")) as file:
        ...
        for line in lines:
            ...
            ent1_list.append(h)
            ent1_list.append(t)
        ent1_list = sorted(list(set(ent1_list)))
    ...
    with open(os.path.join(data_dir, "t_triples")) as file:
        lines = file.read().strip().split("\n")
        ent2_list = []
        rel2_list = []
        for line in lines:
            ...
            ent2_list.append(h)
            ent2_list.append(t)
        ent2_list = sorted(list(set(ent2_list)))
    ...
    with open(os.path.join(data_dir, "ent_ILLs")) as file:
        lines = file.read().strip().split("\n")
        align_lines = []
        for line in lines:
    ...
    name_pseudo_fn = os.path.join(data_dir, "name_pseudo_mappings.txt")
    if os.path.exists(name_pseudo_fn):
        with open(name_pseudo_fn) as file:
            lines = file.read().strip().split("\n")
            pseudo_lines = []
            for line in lines:
                ent1, ent2 = line.split("\t")
                new_line = f"{ent1_oldid2newid_map[ent1]}\t{ent2_oldid2newid_map[ent2]}"
                pseudo_lines.append(new_line)
        with open(os.path.join(out_dir, "name_pseudo_mappings.txt"), "w+") as file:
            file.write("\n".join(pseudo_lines))


    with open(os.path.join(data_dir, "rel_ILLs")) as file:
        lines = file.read().strip().split("\n")
        align_lines = []
        for line in lines:
            rel1, rel2 = line.split("\t")
            new_line = f"{rel1_oldid2newid_map[rel1]}\t{rel2_oldid2newid_map[rel2]}"
            align_lines.append(new_line)
    with open(os.path.join(out_dir, "alignment_of_relation.txt"), "w+") as file:
        file.write("\n".join(align_lines))
...
if __name__ == "__main__":
    ...
    transform_dbp15k_to_uniform(args.data_dir, args.kgids.split(","), args.out_dir)
