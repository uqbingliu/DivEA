...
def eval_alignment_by_sim_mat2(sim_mat, top_k, nums_threads, accurate=False,output = True):
    t = time.time()
    # sim_mat = sim_handler(embed1, embed2, csls, nums_threads)
    ref_num = sim_mat.shape[0]
    t_num = [0 for k in top_k]
    ...
    metrics = {"mr": t_mean, "mrr": t_mrr}
    for idx in range(len(top_k)):
        metrics[f"recall@{top_k[idx]}"] = float(acc[idx])
    return t_prec_set, hits1, metrics
